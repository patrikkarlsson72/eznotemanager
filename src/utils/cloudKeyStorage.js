...
// Function to change the password for the cloud key
export async function changeKeyPassword(userId, key, oldPassword, newPassword) {
  try {
    // First verify the old password by attempting to get the key
    const verifiedKey = await getKeyFromCloud(userId, oldPassword);
    if (!verifiedKey) {
      throw new Error('Current password is incorrect');
    }

    // Store the key with the new password
    const success = await storeKeyInCloud(userId, key, newPassword);
    return success;
  } catch (error) {
