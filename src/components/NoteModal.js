...
const NoteModal = ({ 
  ...
  createdAt,
  updatedAt,
  isEncrypted: initialIsEncrypted = false
}) => {
  ...
  useEffect(() => {
    const loadContent = async () => {
      console.log('NoteModal content updated:', { initialTitle, initialContent, initialIsEncrypted });
    ...
    loadContent();
  }, [initialTitle, initialContent, selectedCategory, initialTags, encryptionKey, initialIsEncrypted]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEncrypted && isEncryptionEnabled && encryptionKey) {
        const encryptedContent = await encryptData(editedContent, encryptionKey);
        finalContent = `encrypted:${encryptedContent}`;
      }
      console.log('Saving note:', { finalTitle, finalContent, selectedCat, tagList, isEncrypted });
      onSave(finalTitle, finalContent, selectedCat, tagList, isEncrypted);
      onRequestClose();
    } catch (error) {
              ...
              <div className="flex items-center gap-2">
                {isEncryptionEnabled && (
                  <button
                    ...
                    title={isEncrypted ? 'Disable encryption for this note' : 'Enable encryption for this note'}
                  >
                    <FontAwesomeIcon icon={isEncrypted ? faLock : faLockOpen} />
                  </button>
                )}
                <button
